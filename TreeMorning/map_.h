...
public:
	...
	node<T1, T2>*erase(const T1& key, node<T1, T2> *nd);
...
template<class T1, class T2>
inline node<T1, T2>* map_<T1, T2>::erase(const T1& key, node<T1, T2>* nd)
{
	if (root == nullptr)
		return root;
	if (key < root->key)
		...
		root->right = delNd(root->right, key);
	else
	{
		...
		root->right = delNd(root->right, tmp->key);
	}
	return root;
}
